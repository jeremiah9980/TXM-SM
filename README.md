...
| File | Purpose |
| --- | --- |
| `index.html` | The whole site. Catalog data lives in the `CATALOG` and `TEAM` constants at the top of the `<script>` block. |
...
```js
{
  ...
  kind: "signal",              // "signal" = runs on a schedule, "standing" = changes with the work
  cadence: "Weekdays 08:00 CT",
